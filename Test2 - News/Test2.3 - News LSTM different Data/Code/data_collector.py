...
def get_news(symbol: str, start: str, end: str, index: int, limit: int = 5, include_content: bool = False) -> list:
    """_summary_
    ...
    if index % 2 == 0:
        ...
        key = os.getenv("ALPACA_API_KEY2")
        secret = os.getenv("ALPACA_API_SECRET2")
...
def collect_data_inkl_news(symbol: str, alpaca_symbol:str, start: str, end:str) -> pd.DataFrame:
    """
    Summary
    ...
    for index, row in df.iterrows():
        ...
        # Update dataframe with the sentiment and probability
        df.loc[index, 'news_probability'] = probability
        df.loc[index, 'news_sentiment'] = sentiment
...
def sentiment_int(row) -> int:
    """_summary_
        Converts String Sentiment into INteger so that the Model can Work with it
    Args:
        row (_type_): Pandas Dataframe row
    ...
    Returns:
        int
    """
    if row["news_sentiment"] == "positive" and row["news_probability"]:
        return 1
    elif row["news_sentiment"] == "negative" and row["news_probability"]:
        return -1
    else:
        return 0